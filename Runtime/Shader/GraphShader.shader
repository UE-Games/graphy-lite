    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            fixed4 frag(v2f IN) : SV_Target
            {
                ...
                if (graphValue > _GoodThreshold) color *= _GoodColor;
                else if (graphValue > _CautionThreshold) color *= _CautionColor;
                else color *= _CriticalColor;

                // Point coloring
                if (graphValue - yCoord > increment * 4) color.a *= yCoord * 0.3 / graphValue;

                // Set as transparent the part on top of the current point value
                if (yCoord > graphValue) color.a = 0;

                // Average white bar
                if (yCoord < Average && yCoord > Average - 0.02) color = fixed4(1, 1, 1, 1);

                // CautionColor bar
                if (yCoord < _CautionThreshold && yCoord > _CautionThreshold - 0.02) color = _CautionColor;

                // GoodColor bar
                if (yCoord < _GoodThreshold && yCoord > _GoodThreshold - 0.02) color = _GoodColor;

                // Fade the alpha of the sides of the graph
                // if (xCoord < 0.03) color.a *= 1 - (0.03 - xCoord) / 0.03;
                // else if (xCoord > 0.97) color.a *= (1 - xCoord) / 0.03;
